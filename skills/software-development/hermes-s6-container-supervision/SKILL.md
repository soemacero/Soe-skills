...
## Security hardening checklist (Hermes-in-Docker)

Run this when auditing or setting up a new Hermes Docker deployment:

```bash
# 1. Verify .env permissions (must be 600, not 644)
ls -la /data/.env
chmod 600 /data/.env

# 2. Confirm running as non-root
whoami && id   # expect: u4s or hermes, NOT root

# 3. Check what's exposed (inside container)
cat /proc/net/tcp | head -20   # ss/netstat may not be available

# 4. Verify Docker isolation
ls /.dockerenv && echo "In container"
cat /proc/1/cgroup | head -3

# 5. Check available resources
free -h    # RAM — important for Ollama model selection
df -h /    # Disk
```

**Key findings from audit pattern:**
- `ss` and `netstat` are often not available in the container — use `/proc/net/tcp` instead
- `sshd_config` does not exist inside the container (SSH is on the host VPS, not inside Docker)
- UFW/iptables typically don't exist inside the container — firewall lives on the host
- The `.env` default permissions from some Hermes versions are `644` (world-readable) — always check and fix to `600`

**Backup script pattern** (`/data/scripts/backup_hermes.sh`):
```bash
#!/bin/bash
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="/data/backups/hermes_$TIMESTAMP.tar.gz"
tar -czf "$BACKUP_FILE" /data/memories/ /data/skills/ /data/config.yaml /data/workspace/.hermes/ 2>/dev/null
ls -t /data/backups/hermes_*.tar.gz 2>/dev/null | tail -n +8 | xargs rm -f 2>/dev/null
echo "[$(date)] Backup: $BACKUP_FILE ($(du -sh $BACKUP_FILE 2>/dev/null | cut -f1))"
```
Schedule via Hermes cron job (`0 3 * * *`). Copy to `~/.hermes/scripts/` for the `no_agent=true` cron pattern.

**Health check pattern** (`~/.hermes/scripts/health_check.sh`):
Check: gateway process running, disk < 90%, `.env` permissions = 600, latest backup < 25h old. Auto-fix `.env` permissions if wrong. Use `no_agent=true` cron, `deliver=origin` so alerts arrive in Telegram only when issues are found (stdout = silent = no message).

See `references/hermes-docker-security-audit.md` for full session transcript and findings.

## Common pitfalls (continued)

### execute_code sandbox redacts API keys in f-strings
The `execute_code` tool's sandbox automatically redacts secret values (API keys, tokens) when they appear in f-strings as `***`. This breaks the string syntax and causes `SyntaxError: unterminated f-string literal`.

**Workaround:** Build auth strings with concatenation, never f-strings:
```python
# WRONG — sandbox redacts and breaks syntax
auth = f"Bearer {api_key}"   # becomes f"Bearer ***" → SyntaxError

# CORRECT — concatenation is not redacted
auth = "Bearer " + api_key
```

This affects any code in `execute_code` that interpolates a secret into a string literal.
