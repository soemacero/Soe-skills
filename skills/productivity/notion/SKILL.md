...
## Zero-Dependency Python Reference

For environments without external tools or packages (e.g. sandboxed containers, serverless scripts, or constrained setups), see the [urllib-based Notion subpage creation pattern](references/urllib-notion-pages.md). This includes boilerplate code for raw blocks and raw markdown ingestion models.
...
⚠️ **Pitfall — PATCH markdown does NOT replace full content:** Sending a complete new markdown body to `PATCH /v1/pages/{id}/markdown` returns HTTP 400. This endpoint appends/partially updates only.

**To delete a section or replace content entirely:**
1. Enumerate blocks: `GET /v1/blocks/{page_id}/children`
2. Identify block IDs to remove (check `type` and `rich_text[].plain_text`)
3. Delete each: `DELETE /v1/blocks/{block_id}` (one request per block)
4. Optionally append new content via `PATCH /v1/blocks/{page_id}/children`

```python
# Python stdlib pattern (avoids shell escaping issues with curl for complex payloads)
import json, urllib.request

auth = "Bearer " + api_key   # build with concatenation, not f-string (execute_code redacts ***)
req = urllib.request.Request(
    f"https://api.notion.com/v1/blocks/{page_id}/children?page_size=100",
    headers={"Authorization": auth, "Notion-Version": "2025-09-03"},
    method="GET"
)
with urllib.request.urlopen(req) as resp:
    blocks = json.loads(resp.read())["results"]

# Delete target blocks
for block in blocks:
    if "text to remove" in str(block):
